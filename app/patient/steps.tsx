...
const emptyTodayData = [{ id: "today-empty", day: "Today", steps: 0, date: new Date().toISOString().split("T")[0] }]
...
const step = () => {
  ...
  const [stepData, setStepData] = useState<StepDataItem[]>(emptyChartData as StepDataItem[])
  ...
  const checkAndResetWeeklyData = async () => {
    try {
      ...
      if (!lastResetInfo || lastResetWeek !== currentWeek || lastResetYear !== currentYear) {
        ...
        await AsyncStorage.setItem("lastStepResetInfo", JSON.stringify(resetInfo))
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        data.forEach((entry: StepDataItem) => {
          ...
          const dayLetter = dayIndex !== -1 ? fixedWeekdays[dayIndex] : entry.day.charAt(0)
          ...
          if (!stepMap.has("latest") || new Date(entry.date) > new Date(stepMap.get("latest").date)) {
            stepMap.set("latest", {
              id: entry.id,
              day: dayLetter,
              steps: entry.steps,
  ...
  const handleSaveGoal = async (steps: string) => {
    ...
    if (!isNaN(parsedSteps) && parsedSteps > 0) {
      ...
      // TODO: Add API call to save the step goal
      // Similar to the weight component's handleSaveWeight function
    }
  }
      ...
      >
        ...
        {hasSteps && (
          <View style={[styles.barWrapper, { height: barHeight }, { width: barWidth }]}>
            <LinearGradient
              colors={isSelected ? ["#5E4FA2", "#5E4FA2"] : ["#B5AED4", "#B5AED4"]}
              style={[
                styles.bar,
                { height: "100%" },
          ...
          >
            {timeRangeOptions.map((option) => (
              ...
              >
                ...
                {timeRange === option.id && <Ionicons name="checkmark" size={16} color="#5E4FA2" />}
              </TouchableOpacity>
            ))}
          </View>
        </TouchableOpacity>
