...
const QuestionnaireManager = ({
    ...
    AsyncStorage.getItem(`intro_shown_${userId}`)
      .then((introShown) => {
        ...
        if (introShown === "true") {
          ...
          console.log(
            "User has seen intro before, will skip to questions after time confirmation"
          );
          setHasSeenIntro(true);
          setIntroStep(4);
  ...
  const sendNextQuestion = () => {
    ...
    if (!sentMessages.has(questionKey)) {
      ...
      const formattedQuestion = formatQuestionConversationally(
        currentDomain,
        currentQuestion
      );
  ...
  const askToContinue = () => {
    ...
    if (!sentMessages.has(continueKey)) {
      ...
      let continueMessage =
        "Would you like to keep going, or shall we pause and continue another time?";
      ...
      saveLastQuestion(
        "continue",
        `continue_${state.currentDomainIndex}`,
        continueMessage
      );
  ...
  const handleUserResponse = (response: string) => {
    ...
    if (hasAskedForTime && !userWantsToStart) {
      ...
      if (
        /yes|yeah|sure|ok|okay|go ahead|proceed|continue|start/i.test(response)
      ) {
        setUserWantsToStart(true);
        return true;
      }
      ...
      if (
        /no|nope|not now|later|busy|can't|cannot|don't have time/i.test(
          response
        )
      ) {
    ...
    if (
      ...
      const pausePatterns = [
        ...
        // Explicit pause phrases with more natural structure
    ...
    if (currentDomain.id === "domain-1") {
      ...
      if (
        ...
        /friend|roommate|temporary|shelter|homeless|couch|staying with/i.test(
          response
        )
      ) {
      ...
      // Question 2: Worried about losing housing
        ...
        /yes|yeah|definitely|worried|concerned|might|could/i.test(response)
      ) {
      ...
      // Question 3: Utility companies
        ...
        /yes|yeah|they have|received notice|shut off|disconnect|threatened/i.test(
          response
        )
      ) {
      ...
      // Question 4: Transportation issues
        ...
        /yes|yeah|sometimes|often|missed|hard|difficult|can't|cannot|no car|no bus/i.test(
          response
        )
      ) {
      ...
      // Question 5: Feel safe at home
        ...
        currentQuestion.id === "q1-5" &&
        /no|not really|sometimes|not always|unsafe|scared/i.test(response)
      ) {
      ...
      // Question 6: Neighborhood concerns
        ...
        /yes|yeah|not safe|dangerous|crime|violence|worried|concerned/i.test(
          response
        )
      ) {
    ...
    // Domain II: Safety & Demographics
      ...
      if (
        ...
        /yes|yeah|hit|hurt|threatened|abused|violence/i.test(response)
      ) {
      ...
      // Question 4: Afraid of partner
        ...
        currentQuestion.id === "q2-4" &&
        /yes|yeah|sometimes|afraid|scared|fear/i.test(response)
      ) {
      ...
      // Question 5: Financial abuse
        ...
        /yes|yeah|sometimes|took|stolen|withheld|controls|won't let me/i.test(
          response
        )
      ) {
    ...
    // Domain III: Education & Employment
      ...
      if (
        ...
        /unemployed|not working|looking|laid off|fired|between jobs/i.test(
          response
        )
      ) {
      ...
      // Question 3: Affording basic needs
        ...
        /yes|yeah|hard|difficult|struggle|can't afford|cannot afford/i.test(
          response
        )
      ) {
      ...
      // Question 4: Help finding job
        ...
        currentQuestion.id === "q3-4" &&
        /yes|yeah|would like|need help|looking/i.test(response)
      ) {
      ...
      // Question 5: Help with school/training
        ...
        currentQuestion.id === "q3-5" &&
        /yes|yeah|interested|would like|need help/i.test(response)
      ) {
      ...
      // Question 6: Daycare needs
        ...
        /yes|yeah|need|better|affordable|quality|childcare|daycare/i.test(
          response
        )
      ) {
        flag = "Childcare need";
    ...
    // Domain IV: Food & Physical Activity
      ...
      if (
        currentQuestion.id === "q4-1" &&
        /yes|yeah|worried|concerned|sometimes|often/i.test(response)
      ) {
      ...
      // Question 2: Food not lasting
        ...
        currentQuestion.id === "q4-2" &&
        /yes|yeah|sometimes|often|ran out|not enough/i.test(response)
      ) {
        flag = "Food insecurity";
      }
      ...
      // Question 3: Access to healthy food
        ...
        /no|not really|hard|difficult|expensive|can't afford|limited/i.test(
          response
        )
      ) {
      ...
      // Question 4: Exercise frequency
        ...
        /never|rarely|once|twice|not much|not often|don't exercise/i.test(
          response
        )
      ) {
    ...
    // Domain V: Home Environment
      ...
      if (
        ...
        /yes|yeah|mold|leak|pest|bug|rat|roach|mouse|heat|cooling|broken|unsafe/i.test(
          response
        )
      ) {
    ...
    // Domain VI: Language & Communication
      ...
      if (
        ...
        /yes|yeah|sometimes|often|need help|difficult|hard to understand/i.test(
          response
        )
      ) {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (
          ...
          QUESTIONNAIRE_DOMAINS[savedState.currentDomainIndex].questions.length
        ) {
          ...
          let continueMessage =
            "Would you like to keep going to the next section, or shall we pause again?";
          ...
          const currentDomain =
            QUESTIONNAIRE_DOMAINS[savedState.currentDomainIndex];
          const currentQuestion =
            currentDomain.questions[savedState.currentQuestionIndex];
  ...
  const findLastAnsweredQuestionAndContinue = (
    parsedState: QuestionnaireState
  ) => {
    try {
      if (parsedState.responses.length > 0) {
        const lastResponse =
          parsedState.responses[parsedState.responses.length - 1];
        console.log(
          "Last answered question:",
          lastResponse.questionId,
          "in domain:",
          lastResponse.domainId
        );

        // Find the domain index
        const domainIndex = QUESTIONNAIRE_DOMAINS.findIndex(
          (d) => d.id === lastResponse.domainId
        );
        if (domainIndex >= 0) {
          // Find the question index
          const questionIndex = QUESTIONNAIRE_DOMAINS[
            domainIndex
          ].questions.findIndex((q) => q.id === lastResponse.questionId);

          if (
            questionIndex >= 0 &&
            questionIndex <
              QUESTIONNAIRE_DOMAINS[domainIndex].questions.length - 1
          ) {
            ...
            const nextQuestion =
              QUESTIONNAIRE_DOMAINS[domainIndex].questions[nextQuestionIndex];
            onQuestionReady(
            ...
            if (nextDomainIndex < QUESTIONNAIRE_DOMAINS.length) {
              ...
              onQuestionReady(
                `Let's continue with ${nextDomain.description.toLowerCase()}. ${
                  firstQuestion.text
                }`
              );
            } else {
